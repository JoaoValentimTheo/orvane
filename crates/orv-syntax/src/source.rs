...
impl SourceFile {
    ...
    /// The 1-based line/column of a byte offset `start` and, if `end` sits on a
    /// later line, of `end` as well. Used by golden `.err` output (§8.1), which
    /// is anchored at the primary span start.
